...
Currently supports Windows 11, macOS on Apple Silicon, and Arch or Ubuntu Linux based distros.
...
Execute winstall.bat on Windows or install.sh on Linux first, then run wupdate.bat on Windows or update.sh on Linux for each subsequent run. On macOS, double click the Macinstall app to both install and update, or run the install.sh/update.sh scripts from the Terminal.
